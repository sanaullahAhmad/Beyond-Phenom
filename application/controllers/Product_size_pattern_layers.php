...
class Product_size_pattern_layers extends CI_Controller
{
    ...
    public function index($size_id=null)
    {
        ...
        $config['total_rows'] = $this->Product_size_pattern_layers_model->total_rows($q, $size_id);
        $product_size_pattern_layers = $this->Product_size_pattern_layers_model->get_limit_data($config['per_page'], $start, $q, $size_id);
        ...
        foreach ($product_size_pattern_layers as $key => $layer) {
            ...
            // $product_size_pattern_layers{$key}->categoryTitle = $cat_info->categoryTitle;
            // $product_size_pattern_layers{$key}->categoryId = $cat_info->categoryId;
        }
        ...
        if($size_id)
        {
            $data['product_size'] = $this->Product_sizes_model->get_by_id($size_id);
            $data['product_category'] = $this->Product_categories_model->get_by_id($data['product_size']->categoryId);
        }
    ...
    public function create($categoryId=null, $sizeId=null) 
    {
        $data = array(
            'button' => 'Create',
            ...
            'patternFileId' => set_value('patternFileId', $sizeId),
            'categoryId' => set_value('categoryId', $categoryId),
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(count($errors)==0){
                ...
                redirect(site_url('product_size_pattern_layers/index/'.$this->input->post('patternFileId',TRUE)));
            }
            else{
