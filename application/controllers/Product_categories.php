...
class Product_categories extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'product_categories/?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'product_categories/?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'product_categories/';
            $config['first_url'] = base_url() . 'product_categories/';
        }
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $data = array();
            ...
            if (!empty($_FILES['categoryBase']['name'])) {
                $this->upload->initialize($config);
                if ($this->upload->do_upload('categoryBase')) {
                ...
                array_push($errors, "Error! categoryBase not selected");
            }
            if (!empty($_FILES['categoryMiddle']['name'])) {
                $this->upload->initialize($config);
                if ($this->upload->do_upload('categoryMiddle')) {
                    $up_data = $this->upload->data();
                ...
                array_push($errors, "Error! categoryMiddle not selected");
            }
            if (!empty($_FILES['categoryTop']['name'])) {
                $this->upload->initialize($config);
                if ($this->upload->do_upload('categoryTop')) {
                    $up_data = $this->upload->data();
                ...
                array_push($errors, "Error! categoryTop not selected");
            }
            if (!empty($_FILES['categoryObjFile']['name'])) {
                $this->upload->initialize($config);
                if ($this->upload->do_upload('categoryObjFile')) {
                    $up_data = $this->upload->data();
                ...
                array_push($errors, "Error! categoryObjFile not selected");
            }
            if (!empty($_FILES['categoryMTLFile']['name'])) {
                $this->upload->initialize($config);
                if ($this->upload->do_upload('categoryMTLFile')) {
                    $up_data = $this->upload->data();
                ...
                array_push($errors, "Error! categoryMTLFile not selected");
            }
            if (!empty($_FILES['categoryModelPatternFile']['name'])) {
                $this->upload->initialize($config);
                if ($this->upload->do_upload('categoryModelPatternFile')) {
                    $up_data = $this->upload->data();
                ...
                array_push($errors, "Error! categoryModelPatternFile not selected");
            }
            $data['categoryTitle']=$this->input->post('categoryTitle',TRUE);
...
public function update_action()
{
    ...
    if ($this->form_validation->run() == FALSE) {
	    ...
	    $directory = dirname(dirname(dirname(__FILE__))).'/public/uploads/product_categories/'.$this->input->post('categoryTitle',TRUE);
        if(!file_exists($directory)){
            $row = $this->Product_categories_model->get_by_id($this->input->post('categoryId', TRUE));
            rename(dirname(dirname(dirname(__FILE__))).'/public/uploads/product_categories/'.$row->categoryTitle, $directory);
        }
